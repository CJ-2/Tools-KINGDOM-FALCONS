...
class InstaClient:
    ...
    def Connect(self):
        if (self.isproxyok != True):
            PrintError("Proxy works!", self.user, self.ip, ":" ,self.port, ")")
            return

        if (self.ip != None and self.port != None):
            PrintSuccess("Proxy works! (Proxy:", self.user ,self.ip, ":" ,self.port, ")")
        self.GetAndUpdate("https://www.instagram.com/accounts/login/")
        if (self.IsCookiesOK() != True):
            PrintError("Can not retrieve cookies! Try another proxy", self.user, self.ip, ":", self.port, ")")
        ...
        if (res.status_code == 200):
            try:
                ...
                if ("message" in obj and obj["message"] == "checkpoint_required"):
                    PrintError("The account requires verification", obj["checkpoint_url"], ")")
                    self.isproxyok = False
                    return
                if ("authenticated" in obj and obj["authenticated"] == True and
                "user" in obj and obj["user"] == True):
                    PrintSuccess("Login successfully!", self.user)
                    self.isproxyok = True
                    return
                if ("errors" in obj and "error" in obj["errors"]):
                    PrintError("Did not log in! proxy does not work", self.user, self.ip, ":", self.port, ")")
                    self.isproxyok = False
                    return
                self.isproxyok = False
            except:
        ...
        try:
            ...
            if ("description" in obj and "status" in obj):
                if (obj["status"] == "ok" and obj["description"] == "Your reports help keep our community free of spam."):
                    PrintSuccess("Complaint successfully submitted", self.user)
                    return
            PrintError("Our request to file a complaint has been denied!", self.user)
        except:
            PrintError("There was an error sending the complaint!", self.user)
